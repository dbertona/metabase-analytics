#!/usr/bin/env python3
"""
Carga Plan Gastos de meses cerrados desde BC jobPlanningUnified → Analytics.

Paridad con sync_historico_mano_obra.py, pero Type <> Resource (G/L, Item, …).

Plan del mes M = líneas del snapshot Unified ClosingMonthCode=M−1 con
planningDate en M (mismo criterio que MdO en producción).

Ejemplos:
  /tmp/ps-sync-venv/bin/python scripts/sync_historico_gastos.py \\
    --company "Power Solution Iberia" --since 2025.12 --until 2026.06

  ANALYTICS_DSN=postgresql://... python scripts/sync_historico_gastos.py --all-companies
"""
...
def fetch_month(
    ...
    filt = (
        ...
        f"and lineTypeEnum ne 'Resource' "
...
def aggregate(
    rows: list[dict[str, Any]], closing: str
) -> dict[tuple[str, str, str, str], dict[str, Any]]:
    """Plan mes M desde snapshot M−1: planningDate en next(closing)."""
    closing_ym = parse_closing(closing)
    if not closing_ym:
        return {}
    target_ym = next_ym(*closing_ym)
    acc: dict[tuple[str, str, str, str], dict[str, Any]] = {}
    for row in rows:
        job = (row.get("jobNo") or "").strip()
        closing_code = (row.get("closingMonthCode") or "").strip()
        nr = (row.get("no") or "").strip()
        type_line = (row.get("lineTypeEnum") or "").strip() or "(sin tipo)"
        if not job or not closing_code or type_line == "Resource":
            continue
        plan_ym = planning_year_month(row.get("planningDate"))
        if plan_ym != target_ym:
            continue
        year, month = plan_ym
        key = (job, closing_code, nr, type_line)
        slot = acc.get(key)
        if not slot:
            slot = {
                "job_no": job,
                "closing_month_code": closing_code,
                "year": year,
                "month": month,
                "nr": nr,
                "type_line": type_line,
...
def upsert_month(
    ...
    closing_ym = parse_closing(closing)
    assert closing_ym
    year, month = next_ym(*closing_ym)
    with conn.cursor() as cur:
        cur.execute(
            """
            DELETE FROM public.bc_historico_gastos_mes
        ...
        psycopg2.extras.execute_values(
            cur,
            """
            INSERT INTO public.bc_historico_gastos_mes
...
def main() -> int:
    ...
    # Closing codes M−1 that produce Plan months 2026.01…2026.07 (como MdO actual)
    ap.add_argument("--since", default="2025.12")
    ap.add_argument("--until", default="2026.06")
